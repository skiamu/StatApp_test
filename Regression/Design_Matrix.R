...
# INPUT: 
#      myYear = years for discrete regressors [vector of string]
#      myYear.contunum = years for continous regressors [vector of string]
#      myInd = discrete regressors [vector of strings]
#      myInd.contunum = continous regressors [vector of strings]
# OUTPUT:
#      XD = design matrix
#      Y = vector of response
Design_Matrix <- function(myYear,
   ...
   # nessuna distinzione tra east asia e sud asia
   for(i in 1:n){
   ...
   if(!is.null(myInd.continum)){
      ...
      # ricordo che nella lista X.discrete l'ultima componente non mi serve,
      # serve solo per il GDP per il calcolo della crescita
      X.list = X.discrete[1:(length(X.discrete)-1)]
   }
   ...
   if(dummy.period){
      # le dummy le definisco solo se ho almeno due intervalli
      # definisco di nuovo tutte le grandezze per evitare sovrascrizioni
      j1 <- length(myInd)
      j2 <- ifelse(!is.null(myInd.continum),length(myInd.continum),0)
   ...
   return(list(Y = Y,# response vector
               XD = XD,# design matrix
               n = n,# number of countries in the analysis
               statistics = M,
               nazioni = nazioni))# quality of the indicators
